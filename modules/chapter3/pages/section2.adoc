...
Learn how to perform the *Post-Import Reconciliation* in this video so you can successfully execute the job on the *Unified UI*.

video::container_migration_post.mp4[align="left",width=800,height=500]
...
- There might be a bug in your Installer file so please remove the last line if it matches: 
+
[source,bash,role=execute]
----
...
[source,bash,role=execute]
----
vim collections/ansible_collections/ansible/containerized_installer/roles/automationcontroller/tasks/postgresql.yml
----
+
[source,bash,role=execute]
----
